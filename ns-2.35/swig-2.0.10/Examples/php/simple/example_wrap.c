...
ZEND_NAMED_FUNCTION(_wrap_print_Foo) {
  SWIG_ResetError();
  if(ZEND_NUM_ARGS() != 0) {
    WRONG_PARAM_COUNT;
  }
...
/* vinit subsection */
{
  ...
  z_var->value.dval = Foo;
  zend_hash_add(&EG(symbol_table), (char*)"Foo", sizeof("Foo"), (void *)&z_var,
  sizeof(zval *), NULL);
}
