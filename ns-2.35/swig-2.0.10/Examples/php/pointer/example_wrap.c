...
ZEND_NAMED_FUNCTION(_wrap_add) {
  ...
  if ( ZVAL_IS_NULL( *args[0] ) ) {
    arg1 = 0;
  } else if ( PZVAL_IS_REF( *args[0] ) ) {
    ...
    tmp1 = (double) Z_DVAL_PP(args[0]);
    /*@SWIG@*/;
    arg1 = &tmp1;
  } else {
    SWIG_PHP_Error( E_ERROR, SWIG_PHP_Arg_Error_Msg(1, Expected a reference) );
  }
  ...
  ZVAL_DOUBLE(*args[0], tmp1 );
  ZVAL_DOUBLE(*args[1], tmp2 );
  ZVAL_DOUBLE(*args[2], tmp3 );
  return;
fail:
  SWIG_FAIL();
}
