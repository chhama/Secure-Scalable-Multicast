...
if (!extension_loaded('struct_value')) {
  if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {
    if (!dl('php_struct_value.dll')) return;
  } else {
    // PHP_SHLIB_SUFFIX gives 'dylib' on MacOS X but modules are 'so'.
    if (PHP_SHLIB_SUFFIX === 'dylib') {
      if (!dl('struct_value.so')) return;
    } else {
      if (!dl('struct_value.'.PHP_SHLIB_SUFFIX)) return;
    }
  }
}
...
class Foo {
	...
	function __set($var,$value) {
		...
		if ($var === 'thisown') return swig_struct_value_alter_newobject($this->_cPtr,$value);
	...
	function __get($var) {
		...
		if ($var === 'thisown') return swig_struct_value_get_newobject($this->_cPtr);
