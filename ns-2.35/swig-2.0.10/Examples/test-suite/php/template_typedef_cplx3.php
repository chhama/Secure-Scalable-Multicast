...
if (!extension_loaded('template_typedef_cplx3')) {
  if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {
    if (!dl('php_template_typedef_cplx3.dll')) return;
  } else {
    // PHP_SHLIB_SUFFIX gives 'dylib' on MacOS X but modules are 'so'.
    if (PHP_SHLIB_SUFFIX === 'dylib') {
      if (!dl('template_typedef_cplx3.so')) return;
    } else {
      if (!dl('template_typedef_cplx3.'.PHP_SHLIB_SUFFIX)) return;
    }
  }
}
...
abstract class template_typedef_cplx3 {
...
class UnaryFunctionBase {
	...
	function __set($var,$value) {
		if ($var === 'thisown') return swig_template_typedef_cplx3_alter_newobject($this->_cPtr,$value);
	...
	function __get($var) {
		if ($var === 'thisown') return swig_template_typedef_cplx3_get_newobject($this->_cPtr);
