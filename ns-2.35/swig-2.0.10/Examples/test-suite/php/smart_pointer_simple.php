...
if (!extension_loaded('smart_pointer_simple')) {
  if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {
    if (!dl('php_smart_pointer_simple.dll')) return;
  } else {
    // PHP_SHLIB_SUFFIX gives 'dylib' on MacOS X but modules are 'so'.
    if (PHP_SHLIB_SUFFIX === 'dylib') {
      if (!dl('smart_pointer_simple.so')) return;
    } else {
      if (!dl('smart_pointer_simple.'.PHP_SHLIB_SUFFIX)) return;
    }
  }
}
...
class Foo {
	...
	function __set($var,$value) {
		...
		if ($var === 'thisown') return swig_smart_pointer_simple_alter_newobject($this->_cPtr,$value);
	...
	function __get($var) {
		...
		if ($var === 'thisown') return swig_smart_pointer_simple_get_newobject($this->_cPtr);
