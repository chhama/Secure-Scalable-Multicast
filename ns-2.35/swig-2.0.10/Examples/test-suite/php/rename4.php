...
if (!extension_loaded('rename4')) {
  if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {
    if (!dl('php_rename4.dll')) return;
  } else {
    // PHP_SHLIB_SUFFIX gives 'dylib' on MacOS X but modules are 'so'.
    if (PHP_SHLIB_SUFFIX === 'dylib') {
      if (!dl('rename4.so')) return;
    } else {
      if (!dl('rename4.'.PHP_SHLIB_SUFFIX)) return;
    }
  }
}
...
class Klass {
	...
	function __set($var,$value) {
		if ($var === 'thisown') return swig_rename4_alter_newobject($this->_cPtr,$value);
	...
	function __get($var) {
		if ($var === 'thisown') return swig_rename4_get_newobject($this->_cPtr);
