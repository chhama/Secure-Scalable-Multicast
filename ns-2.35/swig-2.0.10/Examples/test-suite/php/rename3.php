...
if (!extension_loaded('rename3')) {
  if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {
    if (!dl('php_rename3.dll')) return;
  } else {
    // PHP_SHLIB_SUFFIX gives 'dylib' on MacOS X but modules are 'so'.
    if (PHP_SHLIB_SUFFIX === 'dylib') {
      if (!dl('rename3.so')) return;
    } else {
      if (!dl('rename3.'.PHP_SHLIB_SUFFIX)) return;
    }
  }
}
...
class Klass {
	...
	function __set($var,$value) {
		if ($var === 'thisown') return swig_rename3_alter_newobject($this->_cPtr,$value);
	...
	function __get($var) {
		if ($var === 'thisown') return swig_rename3_get_newobject($this->_cPtr);
