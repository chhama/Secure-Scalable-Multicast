...
public:
    SwigDirector_A(PyObject *self, std::complex< int > i, double d = 0.0);
    SwigDirector_A(PyObject *self, int i, bool j = false);
...
public:
    SwigDirector_MyClass(PyObject *self, int a = 0);
...
/* VTable implementation */
    PyObject *swig_get_method(size_t method_index, const char *method_name) const {
      ...
      if (!method) {
        ...
        method = PyObject_GetAttr(swig_get_self(), name);
        if (!method) {
          std::string msg = "Method in class MyClass doesn't exist, undefined ";
          msg += method_name;
          Swig::DirectorMethodException::raise(msg.c_str());
        }
        vtable[method_index] = method;
